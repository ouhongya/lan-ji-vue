  ...
  <el-card class="card-layout" align="center">
    <el-form ref="bigStar" :model="bigStar" class="form-layout" label-width="80px">
      ...
      <el-form-item label="名称:" prop="name"
       ...
       ">
        <el-input v-model="bigStar.name" type="text" maxlength="10" show-word-limit placeholder="请输入大咖名称"/>
      </el-form-item>
...
<script>
  import {findBigStarById, updateBigStar} from '@/api/celebrity/index'
  ...
  export default {
    ...
    methods: {
      ...
      submit(bigStar) {
        this.$refs[bigStar].validate((value) => {
          if (value) {
            updateBigStar(this.bigStar).then(res => {
              if (res.queue.code === 1) {
    ...
    created() {
      findBigStarById(this.$route.query.id).then(res => {
        if (res.queue.code === 1) {
