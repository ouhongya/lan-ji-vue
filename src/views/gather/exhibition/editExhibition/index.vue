  ...
  <el-card class="card-layout">
    ...
    <el-row>
      <el-col :span="24" v-show="count===3" align="center">
        <br/>
        <tinymce v-model="farm.way" :height="500"  style="width: 350px"/>
      </el-col>
    </el-row>
    <el-row>
      <el-col :span="24" v-show="count===4">
        ...
        <el-form :model="farm" label-width="120px" ref="farm" :rules="rulesTime"
          ...
          <el-form-item label="详细地址:" prop="address">
            <el-input placeholder="请输入详细地址" v-model="farm.address"  type="text" size="small" maxlength="20" show-word-limit style="width: 100%">
...
<script>
  import {
    ...
    exhibitionHallList,
    findExhibitionById,
    findExhibitionDetails,
    updateExhibitionDetails,
    updateExhibition,
  } from '@/api/exhibition/index'
  ...
  export default {
    ...
    methods: {
      ...
      nextSubmit(user) {
        if (this.farm.price === null) {
          this.farm.price = `0`
        }
        var formData = new FormData();
        formData.append('eid', this.$route.query.id)
        formData.append('details', this.editorContent)
        updateExhibitionDetails(formData).then(res => {
        })
        let obj1 = {
        ...
        updateExhibition(obj1).then(res => {
          if (res.queue.code === 1) {
    ...
    created() {
      ...
      findExhibitionDetails(this.$route.query.id).then(res => {
        if (res.queue.code === 1) {
          this.editorContent = res.data
        }
      }),
        findExhibitionById(this.$route.query.id).then(res => {
          if (res.queue.code === 1) {
            let data = res.data
            ...
            if (data.expenses == 0) {
              this.farm.resource = 0
              this.farm.price = ''
            } else {
              this.farm.resource = 1
              this.farm.price = data.expenses
            }
            this.form.fileList.push(obj)
            this.url = data.avatar
            this.headerImage = data.avatar
            this.form.id = data.id
            this.form.exhibition = data.exhibitionHallId
            this.form.value = data.exhibitionType
            data.tagList.forEach(item => {
              this.form.tags.push(item.id)
            })
            this.farm.way = data.signUpWay
            this.farm.address = data.address
            this.farm.startTime = data.startTime
            this.farm.endTime = data.endTime
            this.farm.intro = data.intro
          }
        })
    },
  }
